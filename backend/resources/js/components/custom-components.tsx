...
type InfoRowProps = {
	...
	className?: string;
	show_dash_line?: boolean;
};

export const InfoRow = ({ label, value, className, show_dash_line = true }: InfoRowProps) => {
	return (
		<div className={cn('flex justify-between text-sm', className)}>
			<span>{label}</span>
			{!!show_dash_line && <span className="mx-2 flex-1 translate-y-[12px] border-t-2 border-dashed" />}
			<span>{value}</span>
		</div>
	);
};
...
export const SectionCard = ({ children, color = 'zinc' }: { children: React.ReactNode; color?: 'red' | 'green' | 'zinc' }) => {
    ...
    return <div className={`space-y-2 rounded border p-4 sm:min-w-md ${borderColor}`}>{children}</div>;
};
